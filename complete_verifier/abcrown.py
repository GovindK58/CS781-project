...
from auto_LiRPA.perturbations import PerturbationLpNorm, PerturbationMyNorm
...
class ABCROWN:
    ...
    def incomplete_verifier(
        ...
        if isinstance(input_x, dict):
            ...
            # in as a dictionary.
            if arguments.Config['specification']['patch_sz']:
                ptb = PerturbationMyNorm(
                    patch_sz=arguments.Config['specification']['patch_sz'],
                    in_channels=arguments.Config['specification']['in_channels'],
                    norm=input_x['norm'],
            ...
            if arguments.Config['specification']['patch_sz']:
                ptb = PerturbationMyNorm(
                    patch_sz=arguments.Config['specification']['patch_sz'],
                    in_channels=arguments.Config['specification']['in_channels'],
                    norm=norm, x_L=data_lb, x_U=data_ub)
            else:
