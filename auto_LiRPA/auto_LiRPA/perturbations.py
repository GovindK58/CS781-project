...
class PerturbationLpNorm(Perturbation):
    ...
    def concretize_matrix(self, x, A, sign):
        # print("Original concretize matrix")
        ...
        if self.norm == np.inf:
            ...
            diff = (x_ub - x_lb) / 2.0
            # with torch.no_grad():
            #     print("diff in center:", torch.abs(center.flatten() - x.flatten()).sum())
            #     print(self.eps, self.x_L is None, self.x_U is None)
            #     print((diff - self.eps).flatten().abs().max())
            if not isinstance(A, eyeC):
    ...
    def concretize_patches(self, x, A, sign):
        # print("Original concretize patches")
        if self.norm == np.inf:
            x_L, x_U = self.get_input_bounds(x, A)
            ...
            if not A.identity:
                ...
                # print(A.shape, matrix.shape, x.shape, deviation.shape)
...
class PerturbationMyNorm(PerturbationLpNorm):
    # Custom perturbation class to support the custom application of holder's inequality
    def __init__(self, patch_sz=3, in_channels = 1, eps=0, norm=np.inf, x_L=None, x_U=None, eps_min=0):
        super(PerturbationMyNorm, self).__init__(eps=eps, norm=norm, x_L=x_L, x_U=x_U, eps_min=eps_min)
        self.patch_sz = patch_sz
        # a conv layer with kernel of size=patch_sz and all 1s
        self.conv_patch = torch.nn.Conv2d(in_channels, 1, patch_sz, stride=1, padding=0, bias=False)
        self.conv_patch.weight.data.fill_(1.0)
        self.max_pool = torch.nn.MaxPool2d(patch_sz, stride=1, padding=0)

    def concretize_matrix(self, x, A, sign):
        print("Modified concretize matrix")
        # print("A: ", A.shape)
        # print("x: ", x.shape)

        assert self.eps > 0, "eps must be non-negative"

        img_shape = x.shape[1:] # E.g. (1,28,28)

        if (img_shape[-1] < self.patch_sz) or (img_shape[-2] < self.patch_sz):
            print("Patch size larger than image size, using original concretize matrix")
            return super(PerturbationMyNorm, self).concretize_matrix(x, A, sign)
        
        if (self.sparse):
            print("Sparse not supported for custom norm, using original concretize matrix")
            return super(PerturbationMyNorm, self).concretize_matrix(x, A, sign)

        x = x.reshape(x.shape[0], -1, 1) # (batch, img_sz, 1)
        ...
        if not isinstance(A, eyeC):
            ...
            # Apply the custom convolution patch
            # take sum over patch for all patches using conv
            # the norm will be max of these ^(1/q)

            if self.norm == 1:
                A_q_norm = self.max_pool(torch.abs(A).reshape(-1, *img_shape)).view(A.shape[0], A.shape[1], -1).max(dim=-1).values
            else:
                A_q = torch.abs(A) ** self.dual_norm   # A^q 
                A_q_conv = self.conv_patch(A_q.reshape(-1, *img_shape))  # (batch*spec, *conv_output) E.g. (2*10,1,28-k+1,28-k+1)
                A_q_conv = A_q_conv.reshape(A.shape[0], A.shape[1], -1) # (batch, spec, conv_otput_sz)
                A_q_norm = A_q_conv.max(dim=-1).values ** (1/self.dual_norm) 

            deviation = A_q_norm * self.eps

            A = A.reshape(A.shape[0], A.shape[1], -1) # (batch, spec, img_sz)
            bound = A.matmul(x) + sign * deviation.unsqueeze(-1) # (batch, spec, 1)

        else:
            # A is an identity matrix. Its norm is all 1.
            bound = x + sign * self.eps 
        bound = bound.squeeze(-1) # (batch, spec)
        # with torch.no_grad():
        #     bound_org = super(PerturbationMyNorm, self).concretize_matrix(x, A, sign)
        #     print("diff in bound:", torch.abs(bound-bound_org).sum(), self.dual_norm, self.norm, self.eps)
        return bound
    
    def concretize_patches(self, x, A, sign):
        print("Modified concretize patches")

        assert self.eps > 0, "eps must be non-negative"

        if (x.shape[-1] < self.patch_sz) or (x.shape[-2] < self.patch_sz):
            print("Patch size larger than image size, using original concretize patches")
            return super(PerturbationMyNorm, self).concretize_patches(x, A, sign)
        
        if (self.sparse):
            print("Sparse not supported for custom norm, using original concretize patches")
            return super(PerturbationMyNorm, self).concretize_patches(x, A, sign)
        ...
        if not A.identity:
            ...
            if self.norm == 1:
                matrix_norm = self.max_pool(torch.abs(matrix).view(-1, *input_shape[1:])).view(matrix.shape[0], matrix.shape[1], -1).max(dim=-1).values
            else:
                conv_out = self.conv_patch((torch.abs(matrix) ** self.dual_norm).view(-1, *input_shape[1:])) # (batch*out_c*out_h*out_w, *conv_output)
                matrix_norm = (conv_out.view(matrix.shape[0], matrix.shape[1], -1)).max(dim=-1).values ** (1/self.dual_norm)

            deviation = matrix_norm * self.eps
            ...
            bound = x + sign * self.eps
        # with torch.no_grad():
        #     bound_org = super(PerturbationMyNorm, self).concretize_patches(x, A, sign)
        #     print("diff in bound:", torch.abs(bound-bound_org).sum(), self.dual_norm, self.norm, self.eps)
        return bound
