CS781 Project: Robustness Verification for spatially correlated perturbations
===================================

This repo is a clone of the alpha-beta-CROWN repository. 

The installation instructions are the same as in the original repo. You should clone our repo and follow the instructions mentioned in the [Installation and Setup](#installation-and-setup) section to install the dependencies.

We have added an extra perturbation class called `PerturbationMyNorm` in the `auto_LiRPA/auto_LiRPA/perturbation.py` file.
We have added support for passing the patch_sz as a configuration option to `abcrown.py`.



For details about the implementation, please refer to the report.

To run the code, please follow the instructions mentioned [here](#instructions) and pass the `patch_sz` (the size of perturbation batch) and `in_channels` (in case of RGB input) parameter either in config file or as a command line argument.
Example:
```bash
conda activate alpha-beta-crown  # activate the conda environment
cd complete_verifier
python abcrown.py --config exp_configs/tutorial_examples/custom_model.yaml --patch_sz 14 --in_channels 3
```
